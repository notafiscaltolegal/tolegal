...
public class ContribuinteController extends BaseController {
    ...
    Logger logger = Logger.getLogger(ContribuinteController.class);
...
//        page--; // Page 1 == 0
        ...
        String cnpjBase = empresaLogada.getNumeroCnpjBase();

        String cnpj = request.getParameter("cnpj");
        cnpj = cnpj != null ? cnpj.replaceAll("[^0-9]", "") : null;
        String nome = request.getParameter("nome");

        List<DTOContribuinte> resultado = cceContribuinteRepository.findContribuintesNFG(page, max, cnpjBase, numrInscricao, cnpj, nome);
        List<DTOContribuinte> contribuintes =  mensagemRepository.retornaQtdMensagensEmpresas(resultado);
        Long count = cceContribuinteRepository.countContribuintesNFG(cnpjBase, numrInscricao);

        Map<String, Object> retorno = new HashMap<String, Object>();
        retorno.put("contribuintes", contribuintes);
        retorno.put("urlBase", "/nfg/contribuinte");
    ...
    public ModelAndView alterar(Integer inscricao, String dataInicio) throws NFGException {
        ...
        contribuinteService.alterarPelaEmpresa(empresaLogada, inscricao, dataInicio, redirectUrl);
    ...
    public ModelAndView cadastrar(Integer inscricao, String dataInicio, String termoDeAcordo) throws NFGException {
        String redirectUrl = "/contribuinte/cadastro";
        contribuinteService.cadastrarPelaEmpresa(empresaLogada, inscricao, dataInicio, termoDeAcordo, redirectUrl);
