...
function ResponseProcessor(options){
    ...
    this.getColunaCadastro = function(element){
        ...
        if(element[8] == 'S'){
            ...
            elementList+="</td>";
            elementList+="<td>"+ (element[9] == 'C' ? 'Contribuinte cadastrado pelo contador' : 'Empresa já se cadastrou no NFG') +"</td>";
        }else if(element[5] == 'S'){
            elementList+="<td>&nbsp;</td>";
            elementList+="<td><button type='button' data-inscricao='"+element[0]+"' data-url-data-limite='"+settings.urlDataLimite+"' data-url-cadastrar='"+settings.urlCadastrar+"' class='btn btn-default btn-sm' data-cadastro='true' onclick='openForm(this);'>Cadastrar no NFG</button></form></td><td>&nbsp;</td>";
        } else{
            elementList+="<td>&nbsp;</td>";
            elementList+="<td>Emitente não pode participar do NFG</td>";
        }
