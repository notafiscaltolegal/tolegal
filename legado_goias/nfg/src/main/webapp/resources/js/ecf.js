     ...
     $("body").on("click", "#btnImportarECF", function(e) {
         ...
         if($('#fileECF').val()!=null && $('#fileECF').val().length>0){
             ...
             if($.inArray(ext, ['zip']) == -1) {
                 alert("Extensão inválida! O arquivo deve estar compactado com extensão .zip");
                 //Pq o codigo abaixo nao tá funcionando? Depois eu vejo
                 //nfg-webMensagens.show(ALERT_TYPES.ERROR, "Extensão inválida! O arquivo deve estar compactado com extensão .zip",true);
                 return;
             }
         }
        ...
        $.ajax({
            ...
            success: function (response) {
                if(response.error!=null){
                    ...
                    nfgMensagens.show(ALERT_TYPES.ERROR,"Importação finalizada com sucesso",true);
                }
            }
        });
    });
