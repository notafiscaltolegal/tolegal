...
function Cidadao(options) {
    ...
    if(options.nomeTela == 'telaInicial'){
        ...
        new ModaisCidadao({cidadao:me,nomeTela:'conclusaoDadosPerfil'});
...
Cidadao.prototype.preValidacaoDoCidadaoPreCadastrado = function( ){
    ...
    $("#btnValidaDadosCidadaoPreCadastrado").click(function(e) {
        //dataGenPessoaFisica deve estar de acordo com os parametros de preValidaDadosCidadao do CidadaoController
...
Cidadao.prototype.trataDadosPreCadastro = function( ){
    ...
    return {cpf: cpf,nomeDaMae: nomeDaMae,dataDeNascimento:  dataDeNascimento,challenge:
        challenge,captchaResponse: captchaResponse, ehCertificado: false,
...
Cidadao.prototype.trataDadosDaPreValidacaoCertificado = function( ){
    ...
    return {cpf: cpf,nomeDaMae: nomeDaMae,dataDeNascimento:  dataDeNascimento,challenge: challenge,
        captchaResponse: captchaResponse, ehCertificado: true,
        emailPreCadastro:emailPreCadastro,
        nomePreCadastro:nomePreCadastro,
        telefonePreCadastro:telefonePreCadastro
    }
...
Cidadao.prototype.trataDadosDaPreValidacao = function( ){
    ...
    return {cpf: cpf,nomeDaMae: nomeDaMae,dataDeNascimento:  dataDeNascimento,
        challenge: challenge,captchaResponse: captchaResponse, ehCertificado: false,
        emailPreCadastro:emailPreCadastro,
        nomePreCadastro:nomePreCadastro,
        telefonePreCadastro:telefonePreCadastro}
}
...
Cidadao.prototype.chamaPreValidadorDeCadastro = function(dataGenPessoaFisica){
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $("#dataNascCidadao").val(dataGenPessoaFisica.dataDeNascimento);
            $("#telCidadao").val(response.telefone).mask("(99) 9999-9999?9");
            ...
            $("#btnValidaDadosCidadaoPreCadastrado").removeClass("btn-submitform");

            //adicionando acao de key 13 para o botao de concluir cadastro
            ...
            //verifica se tem ou nao senha homologada e se a credencia esta Ativa.
            // sempre quando for inativa, o user pode entrar com uma senha nova
            // se for ativo, quer dizer que ele tem uma senha atual (caso contrario o server lanca uma excecao).
            if(response.senhaCadastrada == null || response.senhaAtiva=='I'){
        ...
        error: function(response){
            if (response.responseJSON.semCredencial){
                window.location.replace(enderecoSite+response.responseJSON.urlRedirect);
                return;
            }
            ...
            $("#btnValidaDadosCidadaoPreCadastrado").css("display","block");
...
Cidadao.prototype.cadastraCidadao = function( ){
    ...
    $("#cadastrarCidadao").click(function(e) {
        ...
        var data = {
            ...
            participaSortreio: $("#checkParticipaSorteio").is(':checked'),
            recebeEmail: $("#checkRecebeEmail").is(':checked')
        }

        ajaxBloqueioDeTela("#cadastrarCidadao",
            enderecoSite + "/cidadao/efetuarcadastro",
            data,
            'POST',
            function(response) {
...
Cidadao.prototype.validaParametrosCadastrarCidadao = function( ){
    ...
    if(email.length > 0 && !ehEmail(email)){
        ...
        nfgMensagens.show(ALERT_TYPES.ERROR, "O E-mail deve coincidir com a sua confirmação!");
    ...
    if(me.enderecoInvalido(cep,tipoLogradouro,nomeLogradouro,numero,bairro,complemento,uf,cidade,false)){
        return false;
    }else if(me.senhaValida(senha,senhaConfirma,senhaAtual,false)){ //VALIDACOES DE SENHA
        return true;
    }
}
...
Cidadao.prototype.carregaEnderecoPorCep = function(){
    ...
    if($("#enderecoHomologado").val()==null || $("#enderecoHomologado").val()!='S'){

        ajaxBloqueioDeTela(
            "#enderecoPanel",
            enderecoSite + "/cidadao/verificaEndereco",
            {
            ...
            function(response) {
                if(!response.cepInvalido){
                    ...
                    if($("#logradouroCidadao").val().length <= 0)
                        $("#logradouroCidadao").val(response.nomeLogradouro);
                    if($("#bairroCidadao").val().length <= 0)
                        $("#bairroCidadao").val(response.nomeBairro);
                    if($("#numeroCidadao").val().length <= 0)
                        $("#numeroCidadao").val(response.numero);
                    if($("#complementoCidadao").val().length <= 0)
                        $("#complementoCidadao").val(response.complemento);

                    me.carregaUFMunicipios(response.uf,response.municipio);
                    me.carregaTipoLogradouro(response.tipoLogradouro);
...
Cidadao.prototype.enderecoInvalido = function(cep,tipoLogradouro,nomeLogradouro,numero,bairro,complemento,uf,cidade,modal){
    ...
    if(cep==null || cep==""){
        ...
        $("#cepCidadao").focus();
    }else if(tipoLogradouro==null || tipoLogradouro=="") {
        nfgMensagens.show(ALERT_TYPES.ERROR, "Escolha o tipo de logradouro!",modal);
        $("#selectLogradouro").focus();
    }else if(nomeLogradouro==null || nomeLogradouro=="") {
        nfgMensagens.show(ALERT_TYPES.ERROR, "Preencha o nome do Logradouro!",modal);
...
Cidadao.prototype.senhaValida = function(senha,senhaConfirma,senhaAtual,modal){
    ...
    if(senha==null || senha==""){
        ...
        nfgMensagens.show(ALERT_TYPES.ERROR, "Preencha o campo de confirmação de Senha !",modal);
        ...
        nfgMensagens.show(ALERT_TYPES.ERROR, "A senha deve conter uma ou mais letras!",modal);
        $("#passCidadao").focus();
    }else if(senha.match(/\d+/) == null) {
        nfgMensagens.show(ALERT_TYPES.ERROR, "A senha deve conter um ou mais números!",modal);
        $("#passCidadao").focus();
    }else if(senhaConfirma!=senha){
        nfgMensagens.show(ALERT_TYPES.ERROR, "A senha deve coincidir com a sua confirmação!",modal);
        $("#passCidadaoConfirm").focus();
    }else{
        return true;
    }
...
Cidadao.prototype.emailRecuperarSenha = function( ){
    var me = this;
    $("#submitEmailRecuperarSenha").click(function(e) {
        $.ajax({
            url: enderecoSite + "/cidadao/recuperarsenha",
            data:{
                cpf: me.removeMascaraCpf($("#cpfRecuperacao").val())
            },
            type: 'POST',
            success: function(response) {
                nfgMensagens.show(ALERT_TYPES.SUCCESS, response.status);
            },
            error: function(response) {
                if (response.responseJSON.semCredencial){
                    window.location.replace(enderecoSite+response.responseJSON.urlRedirect);
...
Cidadao.prototype.listarPremiacao = function() {
    var me = this;
    me.pagination = new NFGPagination({
...
Cidadao.prototype.carregaDadosPremiacao = function(){
    var me = this;
    ajaxBloqueioDeTela("#dadosPremiacao", enderecoSite + "/cidadao/carregaDadosDaPremiacao", {}, 'POST', function(response){
        if(response.isUsuarioPremiado){
            $("#textUsuarioPremiado").html("Você foi sorteado!");
            me.listarPremiacao();
        }else{
            $("#textUsuarioPremiado").html("Você não foi sorteado.");
        }
    });
}

Cidadao.prototype.dadosMeuPlacar = function( ){
    ajaxBloqueioDeTela("#dadosMeuPlacar",enderecoSite + "/cidadao/meuPlacar",{},'POST',function(response) {
        if(response.numeroDeNotas!=null){
            $("#numeroDeNotasMeuPlacar").html(response.numeroDeNotas);
            $("#pontosProximoSorteio").html(response.pontosProximoSorteio);
            $("#totalPremiacaoMeuPlacar").html(response.totalPremiacaoMeuPlacar);
        }
...
Cidadao.prototype.concluirPerfil = function( ){
    var me = this;
    $("#btnConclusaoPerfil").click(function(e) {
        var email =$("#emailCidadaoPerfil").val();
        if(!ehEmail(email)){
            nfgMensagens.show(ALERT_TYPES.ERROR, "E-mail inválido!",true);
            return;
        }
        ...
        if(me.enderecoInvalido(cep,tipoLogradouro,nomeLogradouro,numero,bairro,complemento,uf,cidade,true)){
            return;
        }
        ...
        var data = {
            cpf: $("#cpfPerfil").val(),
            email: email,
            telefone: me.removeMascaraTelefone($("#telCidadaoPerfil").val()),
            participaSortreio: $("#checkParticipaSorteio").is(':checked'),
            recebeEmail: $("#checkRecebeEmail").is(':checked'),
        ...
        ajaxBloqueioDeTela("#btnConclusaoPerfil",
            enderecoSite + "/cidadao/gravaConclusaoPerfil",
            data,
            'POST',
            function(response) {
                window.location.replace(enderecoSite+response.urlRedirect);
...
Cidadao.prototype.gravaPerfil = function( ){
    var me = this;
    $("#atualizarPerfil").click(function(e) {
        ...
        ajaxBloqueioDeTela("#atualizarPerfil",
            enderecoSite + "/cidadao/gravaPerfil",
            data,
            'POST',
            function(response) {
                window.location.replace(enderecoSite+response.urlRedirect);
    ...
    if(sorteioId!=null){
        ajaxBloqueioDeTela("#dadosSorteioSelecionado",
            ...
            function(response){
                ...
                $("#textSorteioRealizado").html(response.sorteio.realizadoBoolean? "Sim":"Não");
                $("#textTotalDocsPeriodo").html(response.totalDocs);
                $("#textTotalPontos").html(response.sorteio.status == "1" ? ("Aguardando geração de bilhetes.").fontcolor("goldenrod") :
                    response.sorteio.status == "2" ? ("Gerando bilhetes.").fontcolor("goldenrod") : response.totalPontos);
                $("#textTotalBilhetes").html(response.sorteio.status == "1" ? ("Aguardando geração de Bilhetes.").fontcolor("goldenrod") :
                ...
                $("#textFimDataRegra").html(response.dataFimRegra);
...
Cidadao.prototype.carregaBancos = function(){
    ...
    return ajaxBloqueioDeTela("#divBanco",
        enderecoSite + "/cidadao/carregaBancosPorCodigo",
        {},
        'POST',
        function(response) {
            ...
            $.each(response.bancos, function() {
                select.append($("<option />").val(this.codigo).text(this.nome));
            });
...
Cidadao.prototype.carregaMunicipios = function(uf){
    if(uf!=null ){
        ...
        return ajaxBloqueioDeTela("#divUfMunicipio",
            enderecoSite + "/cidadao/carregaCidadesPorUf",
            {
                uf: uf
            },
            'POST',
            function(response) {
                ...
                $.each(response.municipios, function() {
                    select.append($("<option />").val(this.codigoMunicipio).text(this.nome));
                });
            }
        )
    }
...
Cidadao.prototype.salvaNovaReclamacao = function(){
    var me= this;
    $("body").on("click", "#btnCriaNovaReclamacao", function() {
        ...
        var dataEmissaoDocFiscal   =  $("#dataEmissaoDocFiscalReclamacao").val();
        ...
        document.getElementById("txtNumeroDocFiscalReclamacao1").innerHTML = numeroReclamacao;
        ...
        document.getElementById("txtNumeroDocFiscalReclamacao2").innerHTML = numeroReclamacao;

        switch (motivoReclamacao) {
            case '1':
                document.getElementById("txtMotivoReclamacao").innerHTML =
                    "Cupom Fiscal/Nota Fiscal/Nota Fiscal de Consumidor Eletrônica com meu CPF não consta no painel Minhas Notas do Portal do Cidadão";
                break;
            case '2':
                document.getElementById("txtMotivoReclamacao").innerHTML = "Não perguntou se eu desejava incluir o CPF na nota/cupom fiscal";
                break;
            case '3':
                document.getElementById("txtMotivoReclamacao").innerHTML = "Exigiu a elaboração de um cadastro para a colocação do CPF";
                break;
            case '4':
            ...
            case '5':
                ...
                break;
            case '6':
                document.getElementById("txtMotivoReclamacao").innerHTML = "Alegou não participar do programa NFGoiana";
                break;
        }

        if(iEReclamacao==null || iEReclamacao.length==0){
            nfgMensagens.show(ALERT_TYPES.ERROR, "Preencha o campo I.E.",true);
        ...
        if(numeroReclamacao==null || numeroReclamacao.length==0){
            nfgMensagens.show(ALERT_TYPES.ERROR, "Preencha o campo "+$("#lblNumCooDoc").html().replace(':','')+".",true);
            $("#numeroDocFiscalReclamacao").focus();
            return;
        }
        if(valorReclamacao==null || valorReclamacao.length==0 || valorReclamacao<=0){
            nfgMensagens.show(ALERT_TYPES.ERROR, "Preencha o campo Valor.",true);
        ...
        if($('#fileReclamacao').val()!=null && $('#fileReclamacao').val().length>0){
            ...
            if($.inArray(ext, ['pdf','png','jpg','jpeg']) == -1) {
                nfgMensagens.show(ALERT_TYPES.ERROR, "Extensão de imagem inválida! O arquivo deve estar em uma das seguinte extensões: jpg, pdf ou png.",true);
                return;
            }
        }
        ...
        me.confirmarReclamacaoBtnSalvar(motivoReclamacao,dataDentroDoPrazo);

        var formData = new FormData($("#formNovaReclamacaoCidadao")[0]);

        formData.append("numeroReclamacao",numeroReclamacao);
        formData.append("tipoDocFiscalReclamacao",tipoDocFiscalReclamacao);
        formData.append("codgMotivo",motivoReclamacao);
        formData.append("dataEmissaoDocFiscal",dataEmissaoDocFiscal);
...
Cidadao.prototype.concluirInclusaoReclamacao = function(formData){
    ...
    $("body").on("click", "#btnSalvarNovaReclamacao", function() {
        bloqueioDeTela("#btnSalvarNovaReclamacao");

        $.ajax({
            url: enderecoSite+"/cidadao/salvarNovaReclamacao",
            data: formData,
            type: 'POST',
            cache: false,
            contentType: false,
...
Cidadao.prototype.alterarReclamacao= function(){
    ...
    $("body").on("click", "#btnAlterarReclamacao", function() {
        ...
        var title        = "<strong>Confirmação</strong>";
        ...
        me.confirmModal = new ConfirmModal(title,"Tem certeza que deseja "+tipoCompl.toLowerCase()+" ?",botaoConfirmar,botaoCancelar);
...
Cidadao.prototype.ConfirmaAlteracaoReclamacao = function(){
    var me= this;
    $("body").on("click", "#btnConfirmaAlteracaoReclamacao", function() {
        ...
        ajaxBloqueioDeTela(this,
            enderecoSite+'/cidadao/alterarSituacaoReclamacao',
            {idReclamacao:idReclamacao,novoCodgTipoCompl:codgTipoCompl,infoReclamacao:comentario},
            'POST',
            function(response){

                if(response.sucesso){
                    nfgMensagens.show(ALERT_TYPES.SUCCESS,"Reclamação atualizada com sucesso. Continue acompanhando-a na listagem de andamento da reclamação.",true);
                }else{
                    nfgMensagens.show(ALERT_TYPES.ERROR,"Erro ao tentar atualizar a situação da reclamação. Tente de novo posteriormente.",true);
                }
...
Cidadao.prototype.confirmarReclamacaoBtnSalvar = function(motivoReclamacao,dataDentroDoPrazo){
    ...
    if(motivoReclamacao == "1"){
        ...
        $("#divConfirmacaoReclamacaoProblemasComEmpresa").css("display", "block");
    }
...
Cidadao.prototype.voltarTelaDeReclamacao = function(){
    $("body").on("click", "#btnVoltarReclamacao", function(){
        var motivoReclamacao = $("input[type='radio'][name='radioMotivoReclamacao']:checked").val();
        ...
        $("#divConfirmacaoReclamacaoNotasNoPainel").css("display", "none");
        $("#divConfirmacaoReclamacaoProblemasComEmpresa").css("display", "none");
        $("#divBtnSalvarNovaReclamacao").css("display", "none");
    });
}

Cidadao.prototype.buscarEmpresaPorIe = function(url, inscricao){
    $.get(url, {inscricao: inscricao}, function(data) {
...
Cidadao.prototype.fluxoDeTelaNovaReclamacao = function(){
    ...
    $("body").on("click", ".radioMotivoReclamacao", function() {
        ...
        $( "#tooltipIEReclamacao" ).tooltip({ content: '<img src="/nfg-web/images/nf_ie_b.png" />' });
        $( "#tooltipDataReclamacao" ).tooltip({ content: '<img src="/nfg-web/images/nf_data_b.png" />' });
        $( "#tooltipNumeroReclamacao" ).tooltip({ content: '<img src="/nfg-web/images/nf_nro_b.png" />' });
        $( "#tooltipValorReclamacao" ).tooltip({ content: '<img src="/nfg-web/images/nf_valor_b.png" />' });
    ...
    $("body").on("click", ".radioTipoDocReclamacao", function() {
        ...
        if($(this).val()== notaFiscal){
            ...
            $( "#tooltipDataReclamacao" ).tooltip({ content: '<img src="/nfg-web/images/nf_data_b.png" />' });
            $( "#tooltipNumeroReclamacao" ).tooltip({ content: '<img src="/nfg-web/images/nf_nro_b.png" />' });
            $( "#tooltipDataReclamacao" ).tooltip({ content: '<img src="/nfg-web/images/nf_valor_b.png" />' });
            ...
            $("#tooltipNumeroReclamacao").css("display", "block");
            $("#tooltipValorReclamacao").css("display", "block");
            $( "#tooltipIEReclamacao" ).tooltip({ content: '<img src="/nfg-web/images/cf_ie_b.png" />' });
            $( "#tooltipDataReclamacao" ).tooltip({ content: '<img src="/nfg-web/images/cf_data_b.png" />' });
            $( "#tooltipNumeroReclamacao" ).tooltip({ content: '<img src="/nfg-web/images/cf_coo_b.png" />' });
            $( "#tooltipValorReclamacao" ).tooltip({ content: '<img src="/nfg-web/images/cf_valor_b.png" />' });
        }
    });
}
