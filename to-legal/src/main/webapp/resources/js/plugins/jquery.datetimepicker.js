 ...
 *
 * Plugin alterado por henrique-rh, adicionado verificaç&#225;o de minDate e maxDate no onBlurValidation
 */
(function( $ ) {
	'use strict';
	var default_options  = {
					...
					if( options.validateOnBlur ) {
						input
							...
							.on('blur.xdsoft', function() {
								...
								} else if(options.minDate != false || options.minDate == 0 || options.maxDate != false) {
                                    var minDate = (options.minDate == false || options.minDate == 0) ? new Date() : Date.parseDate(options.minDate, options.format)
                                    minDate.setHours(0); minDate.setMinutes(0); minDate.setSeconds(0); minDate.setMilliseconds(0);
                                    var maxDate = (options.maxDate == false) ? null : Date.parseDate(options.maxDate, options.format);
                                    maxDate.setHours(23); maxDate.setMinutes(59); maxDate.setSeconds(59); maxDate.setMilliseconds(999);
                                    var actualDate = Date.parseDate($(this).val(), options.format);
                                    if(actualDate.getTime() < minDate.getTime() || actualDate.getTime() > maxDate.getTime()) {
                                        $(this).val(options.mask ? options.mask.replace(/[0-9]/g,'_') : null)
                                        datetimepicker.data('xdsoft_datetime').empty();
                                    }
                                } else {
                                    datetimepicker.data('xdsoft_datetime').setCurrentTime($(this).val());
